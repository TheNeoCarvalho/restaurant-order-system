...
let ParsePositiveIntPipe = class ParsePositiveIntPipe {
    transform(value, metadata) {
        ...
        if (isNaN(val)) {
            throw new common_1.BadRequestException(`${metadata.data} deve ser um número`);
        }
        if (val <= 0) {
            throw new common_1.BadRequestException(`${metadata.data} deve ser um número positivo`);
        }
        return val;
    }
};
